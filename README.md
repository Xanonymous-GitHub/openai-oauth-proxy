...
Unknown fields and unsupported endpoints fail explicitly. Documented OpenAI top-level request fields are shape-validated and accepted; fields without a Codex App Server equivalent are compatibility no-ops. This includes sampling and penalty controls, log probabilities, token/choice limits, stop sequences, audio/output modalities, prediction, verbosity, web search options, OpenAI resource metadata/lifecycle controls, moderation, prompt-cache hints, client identifiers, stream obfuscation, and truncation.

Responses `input` items are the one exception to the fail-explicitly rule. Documented item types the App Server cannot reproduce — `additional_tools`, `item_reference`, `compaction`, hosted-tool calls and their outputs, `custom_tool_call`, `local_shell_call`, `shell_call`, `apply_patch_call`, `program`, the MCP items, historical `reasoning`, and completed `function_call` and `function_call_output` items — are accepted and dropped before translation, because agent clients replay their own transcript verbatim and a rejected history item would end the conversation rather than degrade it. Codex cannot safely replay opaque encrypted reasoning or its legacy function-call format in a fresh thread. Dropping is lossy: history those items carried is not visible to the model. Only `message` items are translated from ordinary Responses history; active function-output continuations use the separate tool-continuation path. Clients must not rely on those fields changing generation or response data. `service_tier` and explicit reasoning efforts, including `none`, are forwarded because Codex supports them; non-`none` efforts must be advertised by the selected model. Streaming Chat honors `stream_options.include_usage`; authoritative usage remains App Server-provided. Embeddings, actual audio output, file APIs, remote image URLs, hosted tools, internal Codex tools, multi-user isolation, and a downstream Codex CLI remain non-goals.
...
- Codex 0.146.0 raw-event limitation: cold `thread/resume` and `thread/fork` disable raw events, so those paths cannot return `reasoning.encrypted_content`. Normal stateless history replay starts a raw-enabled thread and is unaffected.
...
The offline gate uses Node 26.7.0, Bun 1.3.14, Python 3.14.7, Codex 0.152.0, the official OpenAI JavaScript client, a real fixture-provider App Server, real pinned Bifrost when Docker is available, and representative OpenCode/Hermes clients. External agent tests are explicitly skipped only when the corresponding binary is absent; CI installs pinned OpenCode 1.18.23 and Hermes Agent 0.20.6 so both are required there. Hermes uses its installer-managed Python 3.11 environment because Hermes 0.20.6 does not support Python 3.14.
