...
Responses `input` items are the one exception to the fail-explicitly rule. Documented item types the App Server cannot reproduce — `additional_tools`, `item_reference`, `compaction`, hosted-tool calls and their outputs, `custom_tool_call`, `local_shell_call`, `shell_call`, `apply_patch_call`, `program`, and the MCP items — are accepted and dropped before translation, because agent clients replay their own transcript verbatim and a rejected history item would end the conversation rather than degrade it. Dropping is lossy: history those items carried is not visible to the model. Only `message`, `reasoning`, `function_call`, and `function_call_output` items are translated. Clients must not rely on those fields changing generation or response data. `service_tier` and explicit reasoning efforts, including `none`, are forwarded because Codex supports them; non-`none` efforts must be advertised by the selected model. Responses preserve requested encrypted reasoning and reasoning-token usage for stateless client replay. Streaming Chat honors `stream_options.include_usage`; authoritative usage remains App Server-provided. Embeddings, actual audio output, file APIs, remote image URLs, hosted tools, internal Codex tools, multi-user isolation, and a downstream Codex CLI remain non-goals.
