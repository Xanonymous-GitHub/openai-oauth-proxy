...
Unknown fields and unsupported endpoints fail explicitly. Embeddings, audio, file APIs, remote image URLs, hosted tools, internal Codex tools, multi-user isolation, and a downstream Codex CLI are non-goals. `max_completion_tokens` and verbosity are translated to explicit developer constraints because App Server has no native fields for them; authoritative usage remains App Server-provided.
